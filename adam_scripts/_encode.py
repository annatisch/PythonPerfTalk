...
def get_encode_method_as_per_amqp_type(amqp_type):
    if amqp_type is None:
        return encode_null
    elif amqp_type == AMQPTypes.boolean:
        return encode_boolean
    elif amqp_type == AMQPTypes.short:
        return encode_short
    elif amqp_type == AMQPTypes.ulong:
        return encode_ulong
    elif amqp_type == AMQPTypes.map:
        return encode_map
    elif amqp_type == AMQPTypes.list:
        return encode_list
    elif amqp_type == AMQPTypes.symbol:
        return encode_symbol
    elif amqp_type == AMQPTypes.described:
        return encode_described
    elif amqp_type == AMQPTypes.ushort:
        return encode_ushort
    elif amqp_type == AMQPTypes.int:
        return encode_uint
    elif amqp_type == AMQPTypes.uint:
        return encode_uint
    elif amqp_type == AMQPTypes.binary:
        return encode_binary
    elif amqp_type == AMQPTypes.string:
        return encode_string
    else:
        return encode_unknown


def encode_value_by_method(output, value, **kwargs):
    encode_method = get_encode_method_as_per_amqp_type(value[VALUE])
    encode_method(output, value[VALUE], **kwargs)
...
def encode_payload_expanded(output, payload):
    # type: (bytearray, Message) -> bytes

    if payload[0]:  # header
        try:
            _ENCODE_MAP[describe_performative(payload[0])[TYPE]](output, describe_performative(payload[0])[VALUE])
        except (KeyError, TypeError):
            encode_unknown(output, describe_performative(payload[0]))

        encode_value(output, describe_performative(payload[0]))

    if payload[2]:  # message annotations
        try:
            _ENCODE_MAP[AMQPTypes.described](
                output,
                (
                    {TYPE: AMQPTypes.ulong, VALUE: 0x00000072},
                    encode_annotations(payload[2])
    ...
    if payload[3]:  # properties
        try:
            _ENCODE_MAP[describe_performative(payload[3])[TYPE]](output, describe_performative(payload[3])[VALUE])
        except (KeyError, TypeError):
            encode_unknown(output, describe_performative(payload[3]))
